...
sub create_pdbx_entry
{
    ...
    $atom_site->{$atom_id}{'pdbx_auth_alt_id'} = $pdbx_auth_alt_id;
...
sub determine_residue_keys
{
    ...
    for my $residue_unique_key ( @current_residue_unique_keys ) {
        ...
        if( exists $residue_key_tree{$reduced_unique_key} ) {
            push @{ $residue_key_tree{$reduced_unique_key} },
                $residue_unique_key;
        } else {
            $residue_key_tree{$reduced_unique_key} =
                [ $residue_unique_key ];
        }
    }
...
sub follow_hetatoms
{
    ...
    my @expanded_atom_ids = ();
    my @next_atom_ids = # Initial set of atom ids.
        map { @{ $_ } }
        grep { defined $_ }
...
sub group_unique_residue_keys
{
    ...
    for my $unique_residue_key_group_ids ( @unique_residue_key_groups_ids ) {
        my @unique_residue_key_group = ();
        for my $unique_residue_group_id ( @{ $unique_residue_key_group_ids } ) {
            push @unique_residue_key_group,
                unique_residue_key( $unique_residue_keys{$unique_residue_group_id} );
        }
