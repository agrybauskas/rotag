...
sub hybridization_new
{
    ...
    for my $atom_id ( keys %{ $atom_site } ) {
        ...
        if( any { $_ eq 'double' } @bond_types ) {
            ...
            $hybridization{$atom_id} = 'sp';
        } else {
            $hybridization{$atom_id} = 'sp3';
        }
    }
...
sub rotatable_bonds_new
{
    my ( $atom_site, $connections, $hybridizations, $options ) = @_;

    my ( $start_atom_id, $next_atom_id ) =
        ( $options->{'start_atom_id'}, $options->{'next_atom_id'} );
    ...
    while( scalar( @next_atom_ids ) != 0 ) {
        ...
        for my $atom_id ( @next_atom_ids ) {
            ...
            if( $hybridizations->{$parent_atom_id} eq 'sp3' ||
                $hybridizations->{$atom_id} eq 'sp3' ) {
                # If last visited atom was sp3, then rotatable bonds from
                # previous atom are copied and the new one is appended.
                push @{ $rotatable_bonds{$atom_id} },
                    [ $parent_atom_id, $atom_id ];
            ...
            # Marks neighbouring atoms.
            push @neighbour_atom_ids, @{ $connections->{$atom_id} };
