...
sub rotation_translation
{
    ...
    for my $residue_unique_key ( @residue_unique_keys ) {
        ...
        for my $atom_id ( keys %{ $residue_site }  ) {
            ...
            if( ! ( exists $bendable_angles->{$atom_id} &&
                    exists $rotatable_bonds->{$atom_id} &&
                    exists $stretchable_bonds->{$atom_id} ) ) { next; }

            my @transf_matrices; # Matrices for transforming atom coordinates.

            # Creates transformation matrices that describes the change of bond
            # lengths.
