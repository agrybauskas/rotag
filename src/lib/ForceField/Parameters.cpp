...
Parameters::Parameters(char* program_file_path) {
    ...
    for (size_t i = 0; i < pdbx.length("_rotag_lennard_jones.type_symbol_1"); i++) {
        std::string type_symbol_1 =
            pdbx.value("_rotag_lennard_jones.type_symbol_1", i);
        std::string type_symbol_2 =
            pdbx.value("_rotag_lennard_jones.type_symbol_2", i);
        double sigma = pdbx.value("_rotag_lennard_jones.sigma", i);
        double epsilon = pdbx.value("_rotag_lennard_jones.epsilon", i);
    ...
    // "_rotag_partial_charge" category.
    for (size_t i = 0; i < pdbx.length("_rotag_partial_charge.label_comp_id"); i++) {
        std::string residue_name =
            pdbx.value("_rotag_partial_charge.label_comp_id", i);
        std::string atom_name =
            pdbx.value("_rotag_partial_charge.label_atom_id", i);
        double partial_charge_value =
            pdbx.value("_rotag_partial_charge.value", i);
    ...
    for (size_t i = 0; i < pdbx.length("_rotag_torsional_atom_names.label_comp_id"); i++) {
        std::string residue_name =
            pdbx.value("_rotag_torsional_atom_names.label_comp_id", i);
        std::string atom_name =
            pdbx.value("_rotag_torsional_atom_names.label_atom_id", i);
        std::string alt_atom_name =
            pdbx.value("_rotag_torsional_atom_names.alt_atom_name", i);
    ...
    for (size_t i = 0; i < pdbx.length("_rotag_torsional.label_atom_1_id"); i++) {
        std::string atom_name_1 =
            pdbx.value("_rotag_torsional.label_atom_1_id", i);
        std::string atom_name_2 =
            pdbx.value("_rotag_torsional.label_atom_2_id", i);
        std::string atom_name_3 =
            pdbx.value("_rotag_torsional.label_atom_3_id", i);
        std::string atom_name_4 =
            pdbx.value("_rotag_torsional.label_atom_4_id", i);
        ...
        double epsilon = pdbx.value("_rotag_torsional.epsilon", i);
        double phase = pdbx.value("_rotag_torsional.phase", i);
        double gamma = pdbx.value("_rotag_torsional.gamma", i);
    ...
    for (size_t i = 0; i < pdbx.length("_rotag_h_bond.type_symbol"); i++) {
        std::string type_symbol =
            pdbx.value("_rotag_h_bond.type_symbol", i);
        double sigma =
            pdbx.value("_rotag_h_bond.sigma", i);
        double epsilon =
            pdbx.value("_rotag_h_bond.epsilon", i);

        this->H_BOND[type_symbol] = {sigma, epsilon};
    }

    // "_rotag_residue_atom_necessity" category.
    for (size_t i = 0; i < pdbx.length("_rotag_residue_atom_necessity.label_comp_id"); i++) {
        std::string residue_name =
            pdbx.value("_rotag_residue_atom_necessity.label_comp_id", i);
        std::string atom_name =
            pdbx.value("_rotag_residue_atom_necessity.label_atom_id", i);
        std::string necessity_value =
            pdbx.value("_rotag_residue_atom_necessity.value", i);
    ...
    for (size_t i = 0; i < pdbx.length("_rotag_clear_hybridization.label_comp_id"); i++) {
        std::string residue_name =
            pdbx.value("_rotag_clear_hybridization.label_comp_id", i);
        std::string atom_name =
            pdbx.value("_rotag_clear_hybridization.label_atom_id", i);
        std::string type =
            pdbx.value("_rotag_clear_hybridization.type", i);

        this->CLEAR_HYBRIDIZATION[residue_name][atom_name].type = type;
    }

    // "_rotag_connectivity" category.
    for (size_t i = 0; i < pdbx.length("_rotag_connectivity.label_comp_id"); i++) {
        std::string residue_name =
            pdbx.value("_rotag_connectivity.label_comp_id", i);
        std::string atom_name_1 =
            pdbx.value("_rotag_connectivity.label_atom_1_id", i);
        std::string atom_name_2 =
            pdbx.value("_rotag_connectivity.label_atom_2_id", i);

        this->CONNECTIVITY[residue_name][atom_name_1].push_back(atom_name_2);
    }

    // "_rotag_hydrogen_names" category.
    for (size_t i = 0; i < pdbx.length("rotag_hydrogen_names.label_comp_id"); i++) {
        std::string residue_name =
            pdbx.value("_rotag_hydrogen_names.label_comp_id", i);
        std::string atom_name =
            pdbx.value("_rotag_hydrogen_names.label_atom_id", i);
        std::string hydrogen_atom_name =
            pdbx.value("_rotag_hydrogen_names.label_hydrogen_atom_id", i);
    ...
    for (size_t i = 0; i < pdbx.length("_rotag_symmetrical_atom_names.label_comp_id"); i++) {
        std::string residue_name =
            pdbx.value("_rotag_symmetrical_atom_names.label_comp_id", i);
        std::string atom_name_1 =
            pdbx.value("_rotag_symmetrical_atom_names.label_atom_1_id", i);
        std::string atom_name_2 =
            pdbx.value("_rotag_symmetrical_atom_names.label_atom_2_id", i);
    ...
    for (size_t i = 0; i < pdbx.length("_rotag_dihedral_angle.label_comp_id"); i++) {
        std::string residue_name =
            pdbx.value("_rotag_dihedral_angle.label_comp_id", i);
        std::string angle = pdbx.value("_rotag_dihedral_angle.angle", i);
        double range_from = pdbx.value("_rotag_dihedral_angle.range_from", i);
        double range_to = pdbx.value("_rotag_dihedral_angle.range_to", i);
        double step = pdbx.value("_rotag_dihedral_angle.step", i);
        std::string type = pdbx.value("_rotag_dihedral_angle.type", i);
    ...
    for (size_t i = 0; i < pdbx.length("_rotag_interaction_atom_names.label_atom_id"); i++) {
        std::string atom_name =
            pdbx.value("_rotag_interaction_atom_names.label_atom_id", i);

        this->INTERACTION_ATOM_NAMES.add("_atom_site.label_atom_id", atom_name);
    }

    // "_rotag_mainchain_atom_names" category.
    for (size_t i = 0; i < pdbx.length("_rotag_mainchain_atom_names.label_atom_id"); i++) {
        std::string atom_name =
            pdbx.value("_rotag_mainchain_atom_names.label_atom_id", i);

        this->MAINCHAIN_ATOM_NAMES.add("_atom_site.label_atom_id", atom_name);
    }

    // "_rotag_sidechain_atom_names" category.
    for (size_t i = 0; i < pdbx.length("_rotag_sidechain_atom_names.label_atom_id"); i++) {
        std::string atom_name =
            pdbx.value("_rotag_sidechain_atom_names.label_atom_id", i);

        this->SIDECHAIN_ATOM_NAMES.add("_atom_site.label_atom_id", atom_name);
    }

    // "_rotag_rotatable_residue_names" category.
    for (size_t i = 0; i < pdbx.length("_rotag_rotatable_residue_names.label_comp_id"); i++) {
        std::string residue_name =
            pdbx.value("_rotag_rotatable_residue_names.label_comp_id", i);
