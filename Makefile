...
VISUAL_TEST_DIFF=${VISUAL_TEST_CASES:${VISUAL_TEST_CASES_DIR}/%.sh=${VISUAL_TEST_OUT_DIR}/%.diff}

.PHONY: clean distclean all

test: ${TEST_DIFF}

visual_test: ${VISUAL_TEST_DIFF}
...
${VISUAL_TEST_OUT_DIR}/%.diff: ${VISUAL_TEST_CASES_DIR}/%.sh ${VISUAL_TEST_OUT_DIR}/%.dat
	@./$< 2>&1 | diff -B -w $(basename $@).dat - > $@; \
	if [ $$? -eq 0 ]; \
	then echo "$<" \
	     | awk '{ printf "%-40s \033[1m[OK]\033[m\n",    $$1 }'; \
